...
PRAGMA_OPTIMIZE_ON:   { this.ignoreNewLine = false; } [ \t]*'#'[ \t]*'pragma'[ \t]+'optimize'[ \t]*'('[ \t]*'on'[ \t]*')'   ;
PRAGMA_OPTIMIZE_OFF:  { this.ignoreNewLine = false; } [ \t]*'#'[ \t]*'pragma'[ \t]+'optimize'[ \t]*'('[ \t]*'off'[ \t]*')'  ;
PRAGMA_INVARIANT_ALL: { this.ignoreNewLine = false; } [ \t]*'#'[ \t]*'pragma'[ \t]+'invariant'[ \t]*'('[ \t]*'all'[ \t]*')' ;
EXTENSION: { this.ignoreNewLine = false; } [ \t]*'#'[ \t]*'extension' ;
COLON: ':' ;
UNIFORM: 'uniform' ;
BUFFER: 'buffer' ;
IN_TOK: 'in' ;
...
LOWP: 'lowp' ;
PRECISION: 'precision' ;
VERSION: { this.ignoreNewLine = false; } [ \t]*'#'[ \t]*'version' ;
